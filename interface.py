...
def pysimp_interface():
    ...
    layout = [[Titlebar(title = "CyrypticConvert", icon = icon_path, background_color= "dark grey")],
    ...
    window = sg.Window('CrypticConvert', layout)
    ...
    while True:
        ...
        if event == 'Encrypt':
            ...
            sg.PopupNoFrame('See below for encrypted text: \n', encrypted, '(Copied to clipboard)', background_color = "grey")
        elif event == "Decrypt":
            key = values['-EKEY-']
            key = key.strip('b')
            fernet = Fernet(key)
            ...
            sg.PopupNoFrame('See below for decrypted text: \n', decrypted, '(Copied to clipboard)', background_color = "grey")
